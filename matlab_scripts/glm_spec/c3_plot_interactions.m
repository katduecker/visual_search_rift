...
\f0\fs24 \cf0 %% Investigating Guided Search using Rapid Invisible Frequency Tagging\
...
\
%% Compute Cohen's f\'b2 grand average from individual subject files\
...
\
%% Main figure: 3 effects (RT main, gui\'d7RT, set32\'d7RT) + Cohen's f\'b2\
% Each effect takes 1 row: [topoplot] [TFR with outline (spans 2 cols)]\
% Cohen's f\'b2 is the last row.\
% Grid: 4 rows \'d7 3 cols; topo in col 1, TFR in cols 2-3.\
\
...
colormap(cm)\
cb = colorbar;\
title('Cohen''s f\'b2 (RT contribution)')\
\
subplot(4,3,11:12)\
...
xlabel('time (s)'); xticks(-1:0.5:0)\
ylabel('frequency (Hz)'); yticks(10:10:30)\
title('Cohen''s f\'b2 (TFR)')\
cb = colorbar;\
caxis([0, 2e-3])\
cb.Label.String = 'Cohen''s f\'b2';\
box off\
colormap(cm)\
\
