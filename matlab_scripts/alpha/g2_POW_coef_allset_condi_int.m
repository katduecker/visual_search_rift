...
addpath(fullfile(pth,'matlab scripts/',"cbrewer/"))
cm = cbrewer('qual','Dark2',9);

%% Check multicollinearity first

reg_short = {'gui','set','tp','rt','rt*gui','rt*set','rt*gui*set','tot','slouch'};
regr_names = {'guided','set size','target present','rt','rt*gui','rt*set','rt*gui*set','tot','slouch'};
...
for s = 1:length(subj)
    ...
    load(fullfile(outpth,subj{s},'Power_GLM_allset_condi_int.mat'),'multi_coll')
...
print(fig,fullfile(plotpth,'multi_collin_condi_model_int'),'-dpng')
...
for s = 1:length(subj)
    ...
    load(fullfile(outpth,subj{s},'Power_GLM_allset_condi_int.mat'),'model_T')
...
print(fig,fullfile(plotpth,'GLM_condi_model_int'),'-dpng')
...
save(fullfile(plotpth,'GLM_stat_allset_condi_int.mat'),'stat','Tregr','regr_names')